...
public class UndoTestUnit {
	...
	/**
	 * Testing viewUndo function of the Undo class
	 * Equivalence partition: Stack<OperationRecord<Task,String>> is not empty and 
	 * contains more than 1 task.
	 */
	@Test
	public void test3() {
		/**
		...
		arrCal.add(temp);
		FloatTask floatTask=new FloatTask("Task1","1","not done",Calendar.getInstance(),"5","description1",Calendar.getInstance(),Calendar.getInstance());
		RepeatedTask repeatTask=new RepeatedTask("Task3","3","not done",Calendar.getInstance(),"2","description2",Calendar.getInstance(),Calendar.getInstance(),arrCal,Calendar.getInstance(),5);
		DeadLineTask deadlineTask=new DeadLineTask("Task2","2","done",Calendar.getInstance(),"1","description2",Calendar.getInstance(),Calendar.getInstance(),Calendar.getInstance());
		/**
		 * Testing
		 */
